#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
#  Security harness for DM-gating + invite links. Boots a throwaway
#  Postgres cluster (Postgres won't run as root, so run as a normal
#  user: su pgtest -c 'bash tools/run-accounts-tests.sh'), applies
#  the Supabase shim + schema.sql + the accounts migration, then runs
#  tools/test-accounts.sql. Exits non-zero on any failure.
# ═══════════════════════════════════════════════════════════════
set -euo pipefail
...
WORK="$(mktemp -d)"; DATA="$WORK/data"; DB="accounts"
...
echo "▶ applying shim + schema.sql + accounts migration"
...
echo "▶ security proofs (tools/test-accounts.sql)"
psql -v ON_ERROR_STOP=1 -h "$WORK" -p "$PORT" -d "$DB" --username=postgres -f "$REPO/tools/test-accounts.sql"

echo
echo "════════════════════════════════════════════════"
echo " ALL ACCOUNTS / INVITES / GATING CHECKS PASSED"
echo "════════════════════════════════════════════════"
